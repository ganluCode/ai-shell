...
TOOLS = [
    ...
    {
        "name": "download_file",
        "description": "从远程服务器下载文件到本地（通过 SFTP）",
        "input_schema": {
            "type": "object",
            "properties": {
                "remote_path": {
    ...
    {
        "name": "upload_file",
        "description": "将本地文件上传到远程服务器（通过 SFTP）",
        "input_schema": {
            "type": "object",
            "properties": {
                "local_path": {
    ...
    while True:
        ...
        for tool_use in tool_use_blocks:
            ...
            if tool_name == "search_terminal_output":
                ...
                # SFTP download
                remote_path = tool_input.get("remote_path", "")
                conn = session.get("connection")
                if conn:
                    from llm_shell.services.sftp import download_file as sftp_download
                    try:
                        local_path = await sftp_download(conn, remote_path)
                        result_text = f"文件已下载到本地: {local_path}"
                    except Exception as e:
                        result_text = f"下载失败: {e}"
                else:
                    result_text = "无法下载：没有活动的 SSH 连接"
                ...
                conn = session.get("connection")
                if conn:
                    from llm_shell.services.sftp import upload_file as sftp_upload
                    try:
                        result = await sftp_upload(conn, local_path, remote_path)
                        result_text = f"文件已上传: {result.get('remote_path', remote_path)}, 大小: {result.get('size', 'unknown')} 字节"
                    except Exception as e:
                        result_text = f"上传失败: {e}"
                else:
                    result_text = "无法上传：没有活动的 SSH 连接"
